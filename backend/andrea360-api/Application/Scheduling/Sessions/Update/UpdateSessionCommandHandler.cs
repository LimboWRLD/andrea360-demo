...
{
    internal sealed class UpdateSessionCommandHandler(IApplicationDbContext context, IMapper mapper) : ICommandHandler<UpdateSessionCommand, GetSessionResponse>
    {
        public async Task<Result<GetSessionResponse>> Handle(UpdateSessionCommand request, CancellationToken cancellationToken)
        {
            var existingSession = await context.Sessions.FindAsync(request.SessionId, cancellationToken);
            if (existingSession is null)
                return Result.Failure<GetSessionResponse>(new Error(
                    "Session.NotFound",
            ...
            if (!locationExists) return Result.Failure<GetSessionResponse>(new Error(
            ...
            if (!serviceExists) return Result.Failure<GetSessionResponse>(new Error(
            ...
            return Result.Success(mapper.Map<GetSessionResponse>(existingSession));
        }
    }
}
